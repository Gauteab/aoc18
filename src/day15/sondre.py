...
def sort_players(xs):
    return sorted(xs.keys(), key=lambda x: (x[1], x[0])) # Sorts the players by turn
...
def play(field, players, units, dmg):
    ...
    count = 0
    for p in ps:
        if p in units: # In case of being killed before turn
            ...
            if can_attack(coord, team, players, turn_over, dmg):
                ...
                continue
    # if count != 0 or turn_over:
    #     play(field, players, second)
    return False
...
#     queue = deque([])
#     seen = set(p)
#     for i, j, k in [(0, -1, 'u'), (-1, 0, 'l'), (1, 0, 'r'), (0, 1, 'd')]:
#         coord = (p[0] + i, p[1] + j)
#         if coord in field and coord not in players:
#             queue.append((coord, k))
#             seen.add(coord)
#     while queue:
#         p, direction = queue.popleft()
#         for i, j in [(0, -1), (-1, 0), (1, 0), (0, 1)]:
#             coord = (p[0] + i, p[1] + j)
#             if coord in players:
